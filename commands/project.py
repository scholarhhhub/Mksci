...
def new(output_dir=""):
    log_path=os.path.join(os.path.dirname(os.path.abspath(os.path.dirname(__file__))),'log')
    logging.basicConfig(filename=os.path.join(log_path,'new.log'), format='%(asctime)s - %(name)s - %(levelname)s -%(module)s:  %(message)s',
                        datefmt='%Y-%m-%d %H:%M:%S ',
                        level=logging.INFO)
    ...
    if len(output_dir) > 1 :
        if not os.path.exists(output_dir):
            ...
            template_path=os.path.join(os.path.dirname(os.path.abspath(os.path.dirname(__file__))),'template.yaml')
...
def init():
    log_path=os.path.join(os.path.dirname(os.path.abspath(os.path.dirname(__file__))),'log')
    logging.basicConfig(filename=os.path.join(log_path,'init.log'), format='%(asctime)s - %(name)s - %(levelname)s -%(module)s:  %(message)s',
                        datefmt='%Y-%m-%d %H:%M:%S ',
                        level=logging.INFO)
    logger = logging.getLogger()
    KZT = logging.StreamHandler()
